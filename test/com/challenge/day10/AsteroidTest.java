...
public class AsteroidTest {
    ...
    private void calculateAndCheckIntermediatePositions(Asteroid origin, Asteroid destination, List<Int2DPoint> expectedResult) {
        System.out.print(String.format("Intermediate position from %s to %s: ", origin.getPosition(), destination.getPosition()));
        List<Int2DPoint> intermediatePositions = origin.getIntermediatePositionsTo(destination);
        System.out.println(intermediatePositions);

        assertEquals(intermediatePositions, expectedResult);
    }

    private List<Int2DPoint> createPositionsListFrom2DArray(int[][] positionArray) {
        List<Int2DPoint> positionsList = new ArrayList<>();

        for(int[] position : positionArray) {
            positionsList.add(new Int2DPoint(position[0], position[1]));
        }
